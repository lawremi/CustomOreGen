...
public interface IAgeSymbol {
     ...
     * 
     * @param controller
     *        The controller to register the logic elements to
     * @param seed
     *        A unique seed generated from the world seed and the symbol
     *        position
     * @param A
     *        unique seed for the symbol call. The seed is based on the age seed
     *        and the order of the symbols,
     *        providing a deterministic way of making the same symbol produce
     *        different results within the same age
     */
    public abstract void registerLogic(IAgeController controller, long seed);

    /**
     * How much instability should be added to the world.
     * This is called every time the symbol is added to the world (if it is
     * stacked).
     * It is not necessary to use this function to add instability for
     * duplicated critical logic.
     * Used to limit stacking (ex. After three crystal symbols, add 100
     * instability every time crystal is added).
     * 
     * @param count
     *        How many times the symbol has been added thus far (first time = 1)
